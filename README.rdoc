...
Promenade is a software production management aid for Ruby developers.
Promenade utilizes a lifecycle methodology to help developers prepare and
...
* {Development}[http://github.com/rubyworks/promenade]
* {Mailing List}[http://googlegroups.com/group/rubyworks-mailinglist]


== HOW IT WORKS

Promenade defines development _processions_ which consist of a series of
named destinations, or _stops_. Developers attach production elements to
destination by defining production service instances in project 
Schedule or *.schedule files. Schedules are written either in YAML or
a Ruby DSL.
...
metadata via ERB embedded tags. Promenade gathers this information using
{.ruby}[http://dotruby.github.com/dotruby], but the data source can be
easily customized to meet the needs of different projects.

With service configuration and metadata in place, using Promenade is simply
a matter of passing a procession name and stop to the +promenade+ command line
tool. For example,

  $ promenade main:document
...
Promenade includes three tracks out of the box. The most significant of
...
In additon to +main+, Promenade includes +site+ and +attn+ tracks which are used
...
Please see http://rubyworks.github.com/promenade for more details on how to
use Promenade, including the creation of custom tracks, stops and service plugins.
Also try the <tt>--help</tt> option to see the promenade command's help
information.


== INSTALLATION

Promenade can, of course, be installed via RubyGems:

  $ gem install promenade

We no longer recommend it, but Promenade can also be installed the
old-fashion way by downloading the .tar.gz package and using
Ruby Setup (See http://setup.rubyforge.org).

  $ tar -xvzf promenade-1.0.0.tar.gz
  $ cd promenade-1.0.0
  $ sudo setup.rb
...
All in all, Promenade works well. There are some rough edges with regards
...
and give Promenade a try please let us know of any issues you encounter.


== HISTORY

Promenade is actaully the offspring of Reap v.10, and was called Syckle for 
