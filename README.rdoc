...
Pitstop is a life-cycle build tool. Pitstop provides a systematic methodology
to help developers prepare and release Ruby projects.
...
you have as fair notion about Pitstop. In short, Pitstop defines _tracks_
along which various project services run. The developer only needs to define
service instances (aka _tasks_) in YAML or Ruby configuration files
in a project's Pitfile, .pitstop/*.pitfile or task/*.pitfile files. 
For example, Pitstop's RubyForge service config looks like this:
...
Actually, the Rubyforge service is no longer used becuase Pitstop moved to
GitHub, but this example is good one becuase it exemplifies how tasks are
defined. These service configs often draw on metadata about a project,
as in the above example's use of <tt><%= name %></tt>. Metadata is defined
using POM (see http://rubyworks.rubyforge.org/pom). Essentially, you add
files to your project's +meta+ directory or +Profile+ file, and the information
becomes available via ERB embedded tags.

With service configuration and metadata in place, using Pitstop is simply
a matter of specifying the *track* and *stop* desired.
...
Where reset marks generated files out-of-date, clean removes minor
products and purge removes all prodcuts.
...
This program is in dedication to a great programmer, mentor and all around
helpful guy, Pit Captain.
