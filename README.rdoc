...
Redline is a lifecycle-oriented Ruby project mangement tool. Redline
...
you have as fair notion of Redline. In short, Redline defines *lines* (or *tracks*)
within which all Redline services run. The developer only needs to define service
instances or *tasks*, as YAML and Ruby configuration files in a project's
+task+ directory with a `.redfile` extension, or in a central +Redfile+ file.
For example, Redline's RubyForge service config looks like this:
...
Actually, this is no longer used becuase Redline moved to GitHub, but this
...
always up-to-date and in-sync. Redline includes three tracks out of the box.
...
In additon to +main+, Redline includes +site+ and +attn+ lines which are used
...
use Redline, including the creation of custom tracks, stops and service plugins.
...
We no longer recommend it, but Redline can also be installed the
...
Redline is actaully the offspring of Reap v10, and was call Syckle for 
...
to the lifecycle-oriented system Redline is today.
